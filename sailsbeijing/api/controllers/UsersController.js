module.exports = {
  ...
  getListTotal: async function (req, res) {
    const newType = req.query.newType;
  ...
  getListMsg: async function (req, res) {
    ...
    const tableName = req.query.tableName;
    if (tableName === 'news') {
      const arr = await News.find({ newType }).sort('id asc');
      return res.json(arr)
    }
    const arr = await eval(tableName).find().sort('id asc');
    res.json(arr)
  },
  ...
  getSearchMsg1: async function (req, res) {
    ...
    if (tableName === 'News') {
      const arr = await News.find({ newType:sortName,title:{contains:keyword} }).sort('id desc').skip(start).limit(5);
      return res.json(arr)
    }
